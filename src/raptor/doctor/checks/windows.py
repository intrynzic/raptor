    ...
    def validate(self) -> ValidationResult:
        ...
        if result is None:
            return ValidationResult(
                ...
                message = "Visual Studio's Windows SDK component is missing!"
            )
        ...
        if not sdk_root.exists():
            return ValidationResult(
                valid = False,
                severity = Severity.ERROR,
                message = "Windows SDK directory is missing!"
            )
        ...
        if not include_root.exists():
            return ValidationResult(
                valid = False,
                severity = Severity.ERROR,
                message = "Windows SDK include directory is missing!"
            )
        ...
        if not (latest_sdk / "um" / "windows.h").exists():
            return ValidationResult(
                ...
                message = "windows.h could not be found in the installed Windows SDK!"
            )
        ...
        if not (latest_sdk / "ucrt" / "stdio.h").exists():
            return ValidationResult(
                valid = False,
                severity = Severity.ERROR,
                message = "Windows universal C runtime headers are missing!"
            )
