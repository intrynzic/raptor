...
def validate() -> ValidationResult:
    if not _is_installed():
        return ValidationResult(valid=False, severity=Severity.ERROR, message="Git for Windows is not installed or could not be found!")

    git_path = _git_path()

    if not _check_git_ver(git_path):
        return ValidationResult(valid=False, severity=Severity.ERROR, message="Incorrect Git for Windows version installed.")

    if not _check_git_install(git_path):
        return ValidationResult(valid=False, severity=Severity.ERROR, message="The installed Git for Windows package is corrupted!")
