...
_DOXYGEN_ARCHIVE_URL: str = f"https://github.com/doxygen/doxygen/releases/download/Release_{str(_DOXYGEN_REQUIRED_VER).replace(".", "_")}/{_DOXYGEN_ARCHIVE_NAME}"
...
def validate() -> ValidationResult:
    if not _is_installed():
        return ValidationResult(
            ...
            message = "Doxygen is not installed or could not be found."
        )
    ...
    if not _check_doxygen_ver():
        return ValidationResult(
            ...
            message = "Incorrect Doxygen version installed."
        )
    ...
    if not _check_doxygen_install():
        return ValidationResult(
            valid = False,
            severity = Severity.ERROR,
            message = "The Doxygen install is corrupted!"
        )
