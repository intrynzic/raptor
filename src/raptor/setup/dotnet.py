...
def validate() -> ValidationResult:
    if not _is_installed():
        return ValidationResult(valid=False, severity=Severity.ERROR, message=".NET SDK is not installed or could not be found!")

    dotnet_path = _dotnet_path()

    if not _check_dotnet_ver(dotnet_path):
        return ValidationResult(valid=False, severity=Severity.WARNING, message="Incorrect .NET SDK version installed.")

    if not _check_dotnet_install(dotnet_path):
        return ValidationResult(valid=False, severity=Severity.ERROR, message="The installed .NET SDK is corrupted!")
